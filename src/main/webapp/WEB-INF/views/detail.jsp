				  ...
				  if (comments.length === 0) {
				    ...
				    $.each(comments, function(index, comment) {
				      ...
				      if (comment.DELETEYN === 'Y') {
				        // 삭제된 댓글: 작성자와 버튼 없이 메시지만 표시
				        html += '  <div class="comment-content deleted" style="color:#999; font-style:italic; padding:10px 0;">삭제된 댓글입니다.</div>';
				      } else {
				        // 일반 댓글: 작성자, 내용, 버튼 모두 표시
				        ...
				        if ('${not empty LOGIN_USER}' == 'true') {
				          html += '    <button class="btn btn-sm btn-outline-secondary" onclick="replyComment(' + comment.COMMENT_UID + ', \'' + comment.USERNAME + '\')">답글</button>';
				        }
